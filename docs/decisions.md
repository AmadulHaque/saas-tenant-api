...
**D18 — Usage is an append-only ledger with signed deltas.** `POST /usage` records `{feature, delta, metadata}`; corrections are negative deltas, never updates. Totals are a `SUM` over `(company_id, feature)`. Usage totals are surfaced on the dashboard (see D23); enforcing limits against them is deliberately left to plan limits (`max_users`/`max_customers`), which are count-based.
...
**D23 — Usage totals on the dashboard, still not limit-enforced.** The dashboard payload now includes `usage: {total_events, by_feature}` (grouped `SUM(delta)` over the ledger). Because the dashboard is cached, `UsageRecord` is observed by `FlushTenantDashboardCache` like the other tenant resources — the ledger is append-only, so a `created` flush is sufficient and there is no bulk-write path to special-case. Limits remain count-based (`max_users`/`max_customers`); metered overage billing would be the trigger to revisit that.
